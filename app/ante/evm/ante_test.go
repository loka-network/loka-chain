...
func (suite *AnteTestSuite) TestAnteHandler() {
	...
	setup := func() {
		...
		err := suite.app.EvmKeeper.SetBalance(suite.ctx, addr, big.NewInt(10000000000), evmtypes.DefaultEVMDenom)
	...
	testCases := []struct {
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, sdkmath.NewInt(100*int64(gas))))
				txBuilder, err := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "loka_567002-1", gas, amount)
				suite.Require().NoError(err)
				return txBuilder.GetTx()
			}, false, false, false,
		},
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(evmtypes.DefaultEVMDenom, sdkmath.NewInt(100*int64(gas))))
				txBuilder, err := suite.CreateTestEIP712TxBuilderMsgSend(from, privKey, "loka_567001-1", gas, amount)
				suite.Require().NoError(err)
				return txBuilder.GetTx()
			}, false, false, false,
		},
...
func (suite *AnteTestSuite) TestAnteHandlerWithDynamicTxFee() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			suite.ctx = suite.ctx.WithIsCheckTx(tc.checkTx).WithIsReCheckTx(tc.reCheckTx)
			err := suite.app.EvmKeeper.SetBalance(suite.ctx, addr, big.NewInt((ethparams.InitialBaseFee+10)*100000), evmtypes.DefaultEVMDenom)
			suite.Require().NoError(err)
