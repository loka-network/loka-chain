...
# Update only the aloka supply entry
jq -r --arg total_supply "$total_supply" '
  .app_state["bank"]["supply"] |=
  (map(if .denom == "aloka" then .amount = $total_supply else . end))
...
for i in $(seq 0 $((NUM_VALIDATORS - 1))); do
    ...
    sed -i.bak 's/^minimum-gas-prices *=.*/minimum-gas-prices = "0.0001aloka"/g' "$APP_TOML"
