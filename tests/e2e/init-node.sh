...
KEY="mykey"
CHAINID="${CHAIN_ID:-loka_567000-1}"
...
# Set client config
lokad config keyring-backend "$KEYRING"
lokad config chain-id "$CHAINID"

# if $KEY exists it should be deleted
lokad keys add "$KEY" --keyring-backend $KEYRING --algo "$KEYALGO"

# Set moniker and chain-id for Loka (Moniker can be anything, chain-id must be an integer)
lokad init "$MONIKER" --chain-id "$CHAINID"
...
node_address=$(lokad keys list | grep "address: " | cut -c12-)
...
lokad add-genesis-account $KEY 100000000000000000000000000aloka,100000000000000000000000000gas --keyring-backend $KEYRING
...
lokad gentx $KEY 1000000000000000000000aloka --keyring-backend $KEYRING --chain-id "$CHAINID"
...
# Collect genesis tx
lokad collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
lokad validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
lokad start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001gas --json-rpc.api eth,txpool,personal,net,debug,web3
